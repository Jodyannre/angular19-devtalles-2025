...
export class ProductsService {
  ...
  updateProductCache( product: Product) {
    ...
    this.productsCache.forEach( (productResponse) => {
      productResponse.products = productResponse.products.map( currentProd => {
        return currentProd.id === productId ? product : currentProd;
      });
    });

    console.log('Product cache updated for product ID:', productId);
  }
  ...
  uploadImages(images?: FileList): Observable<string[]> {
    ...
    return forkJoin(uploadObservables).pipe(
      tap( imagesNames => console.log('Uploaded images:', imagesNames))
    )
  }
